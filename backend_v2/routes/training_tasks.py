...
from backend_v2.models import TrainingTask, TrainingTaskProgress
...
def _serialize_task(t: TrainingTask, my: TrainingTaskProgress = None):
    return {
        "id": t.id,
        "category": t.category,
        "level": t.level,
...
def list_tasks(_user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for t in tasks:
        ...
        g["tasks"].append(_serialize_task(t, my))
